...
describe('Lexer', function () {
    const expand = Lexer.expandDefinitions;
    it('#expandDefinitions', function () {
        const definitions = {
            AA: "00",
            BB: "11",
        };
        expect(expand(definitions, "")).eq("");
        expect(expand(definitions, "    ")).eq("    ");
        expect(expand(definitions, "+@AA+")).eq("+(?:00)+");
        expect(expand(definitions, "+@@AA+")).eq("+(?<AA>00)+");
        expect(expand(definitions, "+\\@AA+")).eq("+\\@AA+");
        expect(expand(definitions, "+@AA+@BB+@AA+")).eq("+(?:00)+(?:11)+(?:00)+");
    });
    ...
    it('#addStateRule() use definitions', function () {
        ...
        expect(lexer.rules.INITIAL[0].expression.source).to.equal('(?:[0-9])\\.(?:[0-9])');
    });

    it('#lex() - echo all', function () {
        var output = '';
        var lexer = new Lexer();
        lexer.actions.echo = function () { // redirect echo to variable
            output += this.lexer.text;
        };
    ...
    it('#reject() - with reject', function () {
        var output = '';    // eslint-disable-line no-unused-vars
        var wordCount = 0;
        var lexer = new Lexer();
        lexer.actions.echo = function () { // redirect echo to variable
            output += this.lexer.text;
        ...
        lexer.addRule(/[^\s]+/, function (lexer) {  // eslint-disable-line no-unused-vars
            wordCount++;
        });
    ...
    it('should expect floats', function () {
        var output = '';
        var lexer = new Lexer();
        lexer.actions.echo = function () {}; // redirect echo to null
        ...
        lexer.addRule('.', function (lexer) { // eslint-disable-line no-unused-vars
            output += 'found a dot\n';
        });
        lexer.setSource('1.1\nexpect floats 2.2\n3.3\n');
        lexer.lex();
    ...
    it('should match C-style quoted strings', function () {
        ...
        lexer.addStateRule('str', '\n', function (lexer) { // eslint-disable-line no-unused-vars
        ...
        lexer.addStateRule('str', /\\[0-9]+/, function (lexer) { // eslint-disable-line no-unused-vars
            throw new Error('Bad escape sequence');
        });
        lexer.addStateRule('str', '\\n', function (lexer) { // eslint-disable-line no-unused-vars
            str += '\n';
        });
        lexer.addStateRule('str', '\\t', function (lexer) { // eslint-disable-line no-unused-vars
            str += '\t';
        });
        lexer.addStateRule('str', '\\r', function (lexer) { // eslint-disable-line no-unused-vars
            str += '\r';
        });
        lexer.addStateRule('str', '\\b', function (lexer) { // eslint-disable-line no-unused-vars
            str += '\b';
        });
        lexer.addStateRule('str', '\\f', function (lexer) { // eslint-disable-line no-unused-vars
            str += '\f';
        });
        lexer.addStateRule('str', '\\(.|\n)', function (lexer) {
            str += lexer.text.substr(1);
        });
        lexer.addStateRule('str', /[^\\\n\"]+/, function (lexer) { // eslint-disable-line no-useless-escape
            str += lexer.text;
        });
