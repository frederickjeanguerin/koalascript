...
Lexer.prototype.expandDefinitions = function (source) {
  for (var defName in this.definitions) {
    ...
    var defNameRe = new RegExp('{' + defName + '}', 'g');
    source = source.replace(defNameRe, '(?:' + defExpression + ')');
    var defAltNameRe = new RegExp('@@' + defName, 'ig');
    source = source.replace(defAltNameRe, `(?<${defName}>${defExpression})`);
    var defAltNameRe = new RegExp('@' + defName, 'ig');
    source = source.replace(defAltNameRe, '(?:' + defExpression + ')');
  }
  return source;
}
